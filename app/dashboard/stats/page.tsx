...
// --- COMPOSANT DE CONTENU (Logique principale) ---
function StatsContent() {
  const searchParams = useSearchParams();
  const compareWith = searchParams.get('compare_with'); // Récupération du paramètre URL
  ...
  useEffect(() => {
    async function fetchStats() {
      ...
      try {
        ...
        const formattedBasins = basins.map((b: any) => ({
          id: b.id || b.entity_name || b.nom_zone || b.name, // ID unique (nom ou code)
          name: b.entity_name || b.nom_zone || b.name, // Nom affiché
          production: b.valeur_production || b.production_tonnes || b.animal_count || 0,
          rendement: b.rendement || (Math.random() * 5).toFixed(1), // Fallback si null
          climat: b.climat || "Tropical",
          region: b.region_name || b.adm1_name1 || "Inconnu",
        }));
        ...
        if (formattedBasins.length > 0) {
            if (compareWith) {
                // Cas 1 : On vient de la carte (clic sur "Voir stats")
                // On cherche le bassin qui correspond au nom passé en paramètre
                const found = formattedBasins.find((b: any) => 
                    b.name.toLowerCase().includes(compareWith.toLowerCase()) || 
                    compareWith.toLowerCase().includes(b.name.toLowerCase())
                );
                
                if (found) {
                    setBassinA(found);
                    // Pour le Bassin B, on prend le premier de la liste qui n'est pas A
                    const other = formattedBasins.find((b: any) => b.id !== found.id);
                    setBassinB(other || formattedBasins[0]);
                } else {
                    // Si pas trouvé (ex: changement d'année où le bassin n'existe pas)
        ...
        setEvolutionData(evol.data || (Array.isArray(evol) ? evol : []));
      } catch (err) {
    ...
    fetchStats();
  }, [currentSector, selectedYear, compareWith]); // On recharge si le param URL change
  ...
  const comparisonRows = [
    ...
    },
    {
      ...
      icon: Box,
      valA: bassinA?.production ? Number(bassinA.production).toLocaleString() : "-",
      valB: bassinB?.production ? Number(bassinB.production).toLocaleString() : "-",
      // Logique de surbrillance : True si A > B, False si B > A
      highlight:
        parseFloat(bassinA?.production || 0) > parseFloat(bassinB?.production || 0),
    },
    {
      label:
      ...
      highlight:
        parseFloat(bassinA?.rendement || 0) > parseFloat(bassinB?.rendement || 0),
    },
    ...(currentSector.id !== "elevage"
      ? [
          {
    ...
    <div className="p-6 lg:p-10 space-y-8">
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            >
              ...
              <p className="text-2xl font-black text-slate-800">
                {(
                  allData.reduce(
                    (acc, b) => acc + parseFloat(b.rendement || 0),
                    0
                  ) / (allData.length || 1)
                ).toFixed(2)}
              </p>
          ...
          {/* 
            ==================================================
            COMPARATEUR DE PERFORMANCE (Avec Scroll vers)
            ==================================================
          */}
          <div id="comparator" className="bg-white p-8 lg:p-12 rounded-[3rem] shadow-xl border border-slate-100 relative overflow-hidden transition-all duration-500">
...
// --- PAGE PRINCIPALE (Avec Boundary Suspense) ---
