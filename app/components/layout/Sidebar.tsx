...
import { MapPin, Leaf, Fish, Dog, X, BarChart3, Map as MapIcon, Loader2, Layers } from 'lucide-react';
            ...
            try {
                ...
                if (isMounted) {
                    ...
                    if (data?.filters?.[backendSector]?.options) {
                        ...
                        if (activeTab === 'agriculture') setFilters(["Tous", "Cacao", "Café", "Maïs", "Banane", "Coton"]);
                        else if (activeTab === 'elevage') setFilters(["Tous", "Bovins", "Porcins", "Volailles", "Caprins"]);
                        else if (activeTab === 'peche') setFilters(["Tous", "Maritime", "Continentale", "Artisanale"]);
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-8 scrollbar-thin scrollbar-thumb-slate-700">
                    ...
                    <section>
                        ...
                        <div className="space-y-2">
                            <Link
                                href="/dashboard"
                                className={`flex items-center gap-3 p-3 rounded-xl transition-all font-medium ${pathname === '/dashboard' ? 'bg-amber-400 text-slate-900 shadow-lg shadow-amber-400/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
                            >
                                <MapIcon size={18} /> Carte Interactive
                            </Link>
                            <Link
                                href="/dashboard/stats"
                                className={`flex items-center gap-3 p-3 rounded-xl transition-all font-medium ${pathname === '/dashboard/stats' ? 'bg-amber-400 text-slate-900 shadow-lg shadow-amber-400/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
                            >
                            ...
                            <div className="space-y-2 mb-6">
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`w-full flex items-center justify-between p-3 rounded-xl transition-all duration-200 border ${activeTab === item.id ? `${item.activeBg} text-white ring-1 ring-inset ring-white/10` : 'border-transparent hover:bg-slate-800/50 text-slate-400'}`}
                                    >
                                        <div className="flex items-center gap-3">
                            ...
                            <div className="px-2 pt-2 border-t border-slate-800/50">
                                <div className="flex items-center justify-between mb-3">
                                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                                        {activeTab === 'agriculture' ? 'Cultures' : activeFilter === 'elevage' ? 'Espèces' : 'Types de pêche'}
                                    </label>
